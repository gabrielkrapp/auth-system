...
describe('MakeUserAdmin router', () => {
  ...
  it('should return 403 if user is not an admin', async () => {
    ...
    const response = await request(app).put(`/users/admin/${id}`).set('Authorization', `Bearer ${token}`);
  ...
  it('should make user admin if user exists and requester is an admin', async () => {
    const token = generateToken(id, "testuser", "admin");
